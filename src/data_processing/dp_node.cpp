...
void DataProcessingNode::callbackParameters (objectify::objectify_paramConfig &config , uint32_t level ) {
    ...
    param.viz_data_res  = config.viz_data_res;
    ...
    param.sensor_noise_sigma = config.sensor_noise_sigma;
    param.sensor_data_catched_cycles = config.sensor_data_catched_cycles;

    param.preproc = config.preproc;
...
void DataProcessingNode::callback_odom_laser(const nav_msgs::OdometryConstPtr &_odom , const sensor_msgs::LaserScanConstPtr &_laser ){
    ...
    for (int i = 0 ; i < nr ; i++ ) {
        ...
        double laser_i;
        if((_laser->ranges[i] < param.sensor_range_max )&&(_laser->ranges[i]>0.18)){
